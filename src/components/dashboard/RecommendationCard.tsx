...
const RecommendationCard = ({ item, status }: RecommendationCardProps) => {
    ...
    return (
        <article className="flex flex-col sm:flex-row bg-card rounded-md">
            <button 
                onClick={() => setOpen(true)}
                aria-label={`View details for ${item?.title}`}
                className={cn(
                    status === "archived" ? "bg-muted hover:bg-muted/80 transition-colors" : "bg-primary hover:bg-primary/90 transition-colors",
            ...
            >
                <Box className="size-10 text-primary-foreground sm:mb-2 sm:mx-auto" aria-hidden="true" />
            </button>
            <div className="flex-1 p-6">
                ...
                <p className="text-muted-foreground mb-3 line-clamp-2 sm:line-clamp-none">{item?.description}</p>
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2" role="list" aria-label="Framework compliance">
                        {item?.frameworks?.map((framework, index) => (
                            <span 
                                key={index} 
                                role="listitem"
                                className="text-xs text-muted-foreground px-2 p-0.5 bg-muted rounded-md font-semibold"
                            >
                                {framework.name}
                            </span>
                        ))}
            ...
            <aside className="bg-muted p-6 rounded-md mx-2 mb-2 sm:my-2 w-full sm:w-auto">
                ...
                <div className="flex gap-2 justify-center items-center mt-4">
                    <h3 className="text-sm font-semibold text-foreground" id={`value-score-${item?.recommendationId}`}>Value score</h3>
