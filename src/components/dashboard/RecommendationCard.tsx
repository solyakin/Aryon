    ...
    return (
        <article className="flex flex-col sm:flex-row bg-white rounded-md">
            <button 
                ...
                className={cn(
                    status === "archived" ? "bg-gray-300 hover:bg-gray-400 transition-colors" : "bg-teal-500 hover:bg-teal-600 transition-colors",
            ...
            <div className="flex-1 p-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0 mb-3">
                    ...
                    <div className="flex items-center gap-4" role="list" aria-label="Cloud providers">
                        <VscAzure className="w-5 h-5 text-gray-600" aria-label="Azure" role="img" />
                        <FaAws className="w-5 h-5 text-gray-600" aria-label="AWS" role="img" />
                        <FaGoogleDrive className="w-5 h-5 text-gray-600" aria-label="Google Cloud" role="img" />
                    </div>
                </div>
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2" role="list" aria-label="Framework compliance">
                        {item?.frameworks?.map((framework, index) => (
                            <span 
                                key={index} 
                                role="listitem"
                                className="text-xs text-gray-600 px-2 p-0.5 bg-gray-100 rounded-md font-semibold"
                            >
                                {framework.name}
                            </span>
                        ))}
            ...
            <aside className="bg-gray-100 p-4 rounded-md mx-2 mb-2 sm:my-2 w-full sm:w-auto">
                <div className="text-center border-b border-gray-200 pb-3 mb-2 space-y-1">
                    ...
                    <p className="text-sm text-gray-600" aria-label={`Approximately ${item?.impactAssessment?.totalViolations} violations per month`}>
